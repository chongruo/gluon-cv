...
ex = Experiment('cls_net_default',
                ingredients=[train_hp, valid_hp, cls_net])
...
class ClassificationEstimator(BaseEstimator):
    def __init__(self, config, logger=None):
        ...
        filehandler = logging.FileHandler(self._cfg.train_hp.logging_file)
        ...
        classes = self._cfg.train_hp.classes
        num_training_samples = self._cfg.train_hp.num_training_samples


        num_gpus = len(self._cfg.train_hp.gpus)
        ...
        lr_decay = self._cfg.train_hp.lr_decay
        lr_decay_period = self._cfg.train_hp.lr_decay_period
        if self._cfg.train_hp.lr_decay_period > 0:
            lr_decay_epoch = list(range(lr_decay_period, self._cfg.train_hp.num_epochs, lr_decay_period))
        else:
            lr_decay_epoch = [int(i) for i in self._cfg.train_hp.lr_decay_epoch.split(',')]
        lr_decay_epoch = [e - self._cfg.train_hp.warmup_epochs for e in lr_decay_epoch]
        ...
        lr_scheduler = LRSequential([
            ...
            LRScheduler(self._cfg.train_hp.lr_mode, base_lr=self._cfg.train_hp.lr, target_lr=0,
                        nepochs=self._cfg.train_hp.num_epochs - self._cfg.train_hp.warmup_epochs,
        ...
        self.optimizer = 'nag'
        self.optimizer_params = {'wd': self._cfg.train_hp.wd, 
                            'momentum': self._cfg.train_hp.momentum, 
                            'lr_scheduler': lr_scheduler}
        if self._cfg.train_hp.dtype != 'float32':
            optimizer_params['multi_precision'] = True

        self.net = get_model(self.model_name, **kwargs)
        self.net.cast(self._cfg.train_hp.dtype)
        if self._cfg.train_hp.resume_params != '':
            net.load_parameters(self._cfg.train_hp.resume_params, ctx = self.ctx)

        # teacher model for distillation training
        if self._cfg.train_hp.teacher is not None and self._cfg.train_hp.hard_weight < 1.0:
            teacher_name = self._cfg.train_hp.teacher
            self.teacher = get_model(teacher_name, pretrained=True, classes=classes, ctx=self.ctx)
            self.teacher.cast(self._cfg.train_hp.dtype)
        ...
        def get_data_rec(rec_train, rec_train_idx, rec_val, rec_val_idx, batch_size, num_workers):
            ...
            jitter_param = 0.4
            lighting_param = 0.1
            input_size = self._cfg.train_hp.input_size
            crop_ratio = self._cfg.train_hp.crop_ratio if self._cfg.train_hp.crop_ratio > 0 else 0.875
        ...
        if self._cfg.train_hp.use_rec:
            self.train_data, self.val_data, self.batch_fn = get_data_rec(self._cfg.train_hp.rec_train, 
                                                          self._cfg.train_hp.rec_train_idx,
                                                          self._cfg.train_hp.rec_val, 
                                                          self._cfg.train_hp.rec_val_idx,
                                                          batch_size, num_workers)
        else:
            self.train_data, self.val_data, self.batch_fn = get_data_loader(self._cfg.train_hp.data_dir, 
                                                             batch_size, num_workers)
        ...
        save_frequency = self._cfg.train_hp.save_frequency
        if self._cfg.train_hp.save_dir and save_frequency:
            save_dir = self._cfg.train_hp.save_dir
            makedirs(save_dir)
        else:
            save_dir = ''
            save_frequency = 0
    ...
    def _fit(self):
        ...
        if self._cfg.train_hp.resume_states != '':
            trainer.load_states(self._cfg.train_hp.resume_states)

        if self._cfg.train_hp.label_smoothing or self._cfg_train_hp.mixup:
            sparse_label_loss = False
        else:
            sparse_label_loss = True
        if distillation:
            L = loss.DistillationSoftmaxCrossEntropyLoss(temperature=self._cfg.train_hp.temperature,
                                                         hard_weight=self._cfg.train_hp.hard_weight,
        ...
        for epoch in range(self._cfg.train_hp.resume_epoch, self._cfg.train_hp.num_epochs):
            tic = time.time()
            if self._cfg.train_hp.use_rec:
            ...
            for i, batch in enumerate(self.train_data):
                ...
                if self._cfg.train_hp.mixup:
                    lam = np.random.beta(self._cfg.train_hp.mixup_alpha, 
                                         self._cfg.train_hp.mixup_alpha)
                    if epoch >= self._cfg.train_hp.num_epochs - self._cfg.train_hp.mixup_off_epoch:
                        lam = 1
                    data = [lam*X + (1-lam)*X[::-1] for X in data]

                    if self._cfg.train_hp.label_smoothing:
                        eta = 0.1
                    else:
                        eta = 0.0
                    label = mixup_transform(label, classes, lam, eta)

                elif self._cfg.train_hp.label_smoothing:
                    hard_label = label
                    label = smooth(label, classes)

                if self.distillation:
                    teacher_prob = [nd.softmax(self.teacher(X.astype(self._cfg.train_hp.dtype, copy=False)) / self._cfg.train_hp.temperature) \
                                    for X in data]

                with ag.record():
                    outputs = [self.net(X.astype(self._cfg.train_hp.dtype, copy=False)) for X in data]
                    if self.distillation:
